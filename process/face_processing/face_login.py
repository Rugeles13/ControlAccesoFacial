            ...
            if self.cont_frame == 48:
                ...
                if len(faces_database) != 0 and not self.comparison and self.matcher is None:
                    ...
                    if self.matcher:
                        ...
                        return face_image, self.matcher, 'Aprobado acceso!'
                    else:
                        return face_image, self.matcher, 'User no aprobado'
                else:
                    return face_image, self.matcher, 'Database vacia'
            else:
                return face_image, self.matcher, 'Esperando frame de la cara'
        else:
